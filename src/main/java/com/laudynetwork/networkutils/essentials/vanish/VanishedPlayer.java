...
public record VanishedPlayer(UUID uuid) {
    ...
    private void checkTable() {
        NetworkUtils.getINSTANCE().getDbConnection().createTableWithPrimaryKey("minecraft_networktutils_essentials_vanish", "uuid", new SQLConnection.TableColumn("uid", SQLConnection.ColumnType.VARCHAR, 35),
    ...
    public boolean isVanished() {
        ...
        return Integer.parseInt(sqlConnection.getIntResultColumn("minecraft_networktutils_essentials_vanish", "uuid", this.uuid.toString()).value().toString()) == 1;
